...
Page({
        ...
        app.ajaxSubmit(app.urlApi.denlu, "GET", { code: res.authCode }, function(res) {
          ...
          app.ajaxSubmit(app.urlApi.p_getUserPower, "GET", null, function(res) {
            ...
            app.ajaxSubmit(app.urlApi.p_select_byid, "GET", { id: id }, function(res) {
              ...
              var gmDate = app.getLocalTime(aa.gmDate);
              var gmUser = aa.gmUser;
              var gm = aa.gm;
  ...
  yes_tj: function(res) {
    ...
    if (this.data.text_yj == null || this.data.text_yj == "") {
      ...
      app.ajaxSubmit(app.urlApi.p_bm_tongyong + "?applyType=true&node=" + node, "POST", { gm: that.data.text_yj, id: that.data.id }, function(res) {
        console.log("得到的列表为", res);
        // dd.alert({
        //   title: '信息',
        //   content: '提交成功,点击确定返回',
  ...
  no_tj: function(res) {
    ...
    if (this.data.text_yj == null || this.data.text_yj == "") {
      ...
      console.log("点击了同意");
      var node = app.userinfo.arr[0].nodeNum;
      app.ajaxSubmit(app.urlApi.p_bm_tongyong + "?applyType=false&node=" + node, "POST", { gm: that.data.text_yj, id: that.data.id, refusalReason: that.data.text_yj }, function(res) {
        console.log("得到的列表为", res);
        // dd.alert({
        //   title: '信息',
        //   content: '提交成功,点击确定返回',
