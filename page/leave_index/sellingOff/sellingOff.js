...
Page({
  ...
  //签字功能---------------------------------------------------------------------------------------结束-------------------
  claimingRemarksValue(e) {
    this.setData({
      claimingRemarks: e.detail.value
    });
  },
  // 确认提交
  ok_tj() {
    ...
    let res = {};//结果的json
    if (that.data.i == 1) {
      dd.showToast({ content: '已提交,请稍后', duration: 3000 });
    } else {
      that.setData({
        i: 1,
        submitLoad: true
      });
      res.claimingLocation = that.data.claimingLocation;//定位
      res.claimingTime = that.data.time;//时间
      res.id = that.data.id;//单子id
      res.claimingRemarks = that.data.claimingRemarks;//备注
      console.log(res);
      app.ajaxSubmit(app.urlApi.le_saveAndOff + "?type=claiming", "POST", res, (e) => {
        console.log(e);
        dd.showToast({
          type: 'success',
          content: '销假成功',
