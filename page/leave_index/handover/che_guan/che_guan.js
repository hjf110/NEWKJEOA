...
Page({
        ...
        app.ajaxSubmit(app.urlApi.le_add_select, "GET", { id: id }, function(e) {//获得单条记录内容
          ...
          if (that.data.s3 == true) { //财务签过字了不允许修改了
            that.setData({
              ...
              time: app.getLocalTime(infoList.vehicleTime)
            });
          } else {
  ...
  ok_tj() {
    const that = this;
    let res = {};//结果的json
    if (that.data.vehiclePic.length == 0) {
      dd.showToast({ content: '请拍摄车辆位置停放照片', duration: 3000 });
      ...
      res.vehiclePic = app.tihuan(that.data.vehiclePic);//车辆停放位置图片
