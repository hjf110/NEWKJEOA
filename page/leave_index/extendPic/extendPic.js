...
Page({
    ...
    app.getUserPower(app.urlApi.le_getUserPower, function() {//通知点进去时先登录
      ...
      app.ajaxSubmit(app.urlApi.zidian, "GET", { type: "leave_category" }, function(e1) {
        ...
        app.ajaxSubmit(app.urlApi.le_add_select, "GET", { id: id }, function(e) {//获得单条记录内容
          ...
          if (infoList.financialPic != null && infoList.financialPic != "") {//财务签名照片
  ...
  takePhoto(res) {//拍照
    ...
    app.takePhoto(val_a, val_b, function(res, res2) {
      ...
      if (type == "a1") {//车辆停放位置照片
        that.setData({
          claimingSupplyPic: ccc,
          claimingSupplyPicShow: ddd
        });
      }
    });
  ...
  delect(res) {//删除
    ...
    if (type == "a1") {//车辆停放位置照片
      that.setData({
        claimingSupplyPic: imglist,
        claimingSupplyPicShow: imglist_show
      });
    }
  ...
  ok_tj() {
    ...
    let res = {};//结果的json
    if (that.data.i == 1) {
      dd.showToast({ content: '请勿重复提交', duration: 3000 });
    } else {
      that.setData({
        i: 1,
        submitLoad: true
      });
      res.claimingSupplyPic = app.tihuan(that.data.claimingSupplyPic);//车辆停放位置图片
      // res.vehicleLocation = that.data.vehicleLocation;//定位
      // res.vehicleTime = that.data.time;//时间
