...
Page({
  ...
  bindPickerChange(e) {
    ...
    app.ajaxSubmit(app.urlApi.cp_list, "POST", { page: this.data.page, limit: this.data.limit, s_arr: arr_p, s_refer: this.data.s_refer }, function(res) {
  ...
  next(res) {
    ...
    if (that.data.lasttype == 0) {
      ...
      var arr_p = JSON.stringify(app.userinfo.arr);
      app.ajaxSubmit(app.urlApi.cp_list, "POST", { page: p, limit: this.data.limit, s_arr: arr_p, s_refer: this.data.s_refer }, function(res) {
        console.log("得到的列表为", res);
        var aa = res.data;
        var list_sp = that.data.list;
        if (aa.length > 0) {
  ...
  to_sp(res) {
    ...
    if (this.data.s_refer == "" || this.data.s_refer == null) {
      dd.navigateTo({
        url: '/page/complain_index/complain_approval/complain_approval?station=1&id=' + id + ""
      });
    } else {
      dd.navigateTo({
        url: '/page/complain_index/complain_approval/complain_approval?type=1&station=1&id=' + id + ""
      });
    }
  }
});
