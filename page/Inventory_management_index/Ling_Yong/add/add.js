...
Page({
  ...
  submit(e) {
    ...
    if(res.oname==""||res.oname==null){
    ...
    }else if(res.spName==""||res.spName==null){
      //  dd.showToast({
      //   content: '适用车型不能为空',
      //   duration: 2000,
      // });
      dd.alert({ title: '警告', content: '领用人不能为空', buttonText: '我知道了'});
    ...
    }else if(res.delRepository==""||res.delRepository==null){
       dd.showToast({
        content: '单位不能为空',
        duration: 2000,
      });
      dd.alert({ title: '警告', content: '领用仓库不能为空', buttonText: '我知道了'});
      ...
      app.ajaxSubmit(app.urlApi.st_add_lingYong, "POST", res, function(e) {/*****************************申请提交接口******************************/
